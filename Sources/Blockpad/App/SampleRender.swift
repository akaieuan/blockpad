...
enum SampleRender {
    static func run(outputDirectory: String) {
        ...
        if let data = SketchExport.renderPNGData(blocks, scale: 2) {
    ...
    static func sampleScene() -> [Block] {
        var blocks: [Block] = []
        var z = 0
        func add(_ kind: BlockKind, _ rect: CGRect, _ text: String = "", color: Int = 0, stroke: Int = 1, seed: UInt64) {
            z += 1
            blocks.append(Block(kind: kind, parentID: nil, rect: rect, text: text,
                                colorIndex: color, strokeIndex: stroke, seed: seed, points: [], z: z))
        }

        add(.frame, CGRect(x: 0, y: 0, width: 1440, height: 900), "Desktop", seed: 11)
        add(.box, CGRect(x: 960, y: 0, width: 480, height: 900), "", color: 1, stroke: 2, seed: 22)
        add(.box, CGRect(x: 984, y: 32, width: 136, height: 40), "All", seed: 33)
        ...
        add(.box, CGRect(x: 1208, y: 800, width: 200, height: 56), "Apply", color: 2, stroke: 2, seed: 77)
