...
# Remove the staging bundle outright rather than just unregistering it.
# Unregistering is not enough: macOS rescans and re-registers anything that
# looks like an app, so a bundle left in build/ comes back as a second copy on
# its own. It is regenerated by the next build, so nothing is lost.
"$LSREG" -u "$ROOT/build/Blockpad.app" >/dev/null 2>&1 || true
rm -rf "$ROOT/build/Blockpad.app"
